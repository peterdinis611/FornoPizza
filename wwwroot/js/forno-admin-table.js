(() => {
  ...
  // wwwroot/js/forno-admin-table.src.js
  var require_forno_admin_table_src = __commonJS({
    "wwwroot/js/forno-admin-table.src.js"() {
      init_lib();
        ...
        return [
          ...
          },
          {
            ...
            cell: function(info) {
              ...
              return "<strong>" + esc(row.name) + '</strong><span class="admin-sub">' + esc(row.phone) + "</span>";
            }
          },
          {
            id: "fulfillment",
            ...
            header: "Sp\xF4sob",
            cell: function(info) {
              return String(info.getValue()).toLowerCase() === "pickup" ? "V\xFDdaj" : "Rozvoz";
            }
          },
          {
            id: "status",
            ...
            cell: function(info) {
              ...
              return '<span class="admin-stamp ' + stampClass(status) + '">' + esc(label) + "</span>";
            }
          },
          {
            id: "total",
      ...
      function createInstance(host, options) {
        ...
        function tableOptions() {
          return {
            ...
            globalFilterFn: function(row, _columnId, filterValue) {
              var q = String(filterValue == null ? "" : filterValue).trim().toLowerCase();
              if (!q) {
                return true;
              }
              return String(row.original.search || "").toLowerCase().includes(q);
            },
        ...
        function bind() {
          ...
          root.querySelectorAll("th[data-col]").forEach(function(th) {
            th.addEventListener("click", function() {
              ...
              var header = group ? group.headers.find(function(h) {
        ...
        function render() {
          ...
          var head = headers.map(function(header) {
            ...
            return '<th data-col="' + esc(header.column.id) + '" class="' + (canSort ? "is-sortable" : "") + '" scope="col">' + esc(label) + sortMark(header) + "</th>";
          }).join("");
          var body;
          if (rows.length === 0) {
            body = '<tr><td colspan="' + headers.length + '" class="admin-muted">\u017Diadne l\xEDstky pre filter.</td></tr>';
          } else {
            body = rows.map(function(row) {
              var cells = row.getVisibleCells().map(function(cell) {
                var html = flexRender(
                  cell.column.columnDef.cell,
                  cell.getContext()
                );
                if (cell.column.id === "id") {
                  return '<td><a href="/admin/orders/' + esc(row.original.id) + '">' + esc(html) + "</a></td>";
                }
              ...
              return '<tr data-id="' + esc(row.original.id) + '">' + cells + "</tr>";
            }).join("");
          }
          var sizeOptions = [5, 10, 20, 50].map(function(n) {
            return '<option value="' + n + '"' + (pagination.pageSize === n ? " selected" : "") + ">" + n + "</option>";
          }).join("");
          root.innerHTML = '<div class="admin-tanstack-toolbar"><label class="admin-tanstack-search"><span>H\u013Eada\u0165</span><input data-admin-search type="search" value="' + esc(globalFilter) + '" placeholder="meno, telef\xF3n, stav\u2026" /></label><p class="admin-muted">' + filtered + ' l\xEDstkov</p></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr>' + head + "</tr></thead><tbody>" + body + '</tbody></table></div><div class="admin-tanstack-pager"><button type="button" class="btn" data-prev' + (canPrev ? "" : " disabled") + '>\u2190 Predo\u0161l\xE1</button><span class="admin-muted">Strana ' + (pagination.pageIndex + 1) + " / " + Math.max(1, pageCount) + '</span><button type="button" class="btn" data-next' + (canNext ? "" : " disabled") + '>\u010Eal\u0161ia \u2192</button><label class="admin-tanstack-size"><span>Na str\xE1nku</span><select data-size>' + sizeOptions + "</select></label></div>";
          bind();
        }
        render();
        return {
